...
class NotificationListenerTest {
  ...
  @Mock INotificador notificadorMock;
  ...
  @Test
  @DisplayName(
      "Case 1: Should receive the publisher notification when an recipe is created, and call the notificator successfully.")
  void aoCriarReceitaSuccess() {
    ...
    verify(notificadorMock, times(1))
        .enviar(eq(usuarioMock.getLogin()), eq("Nova Receita"), any(String.class));
  }

  @Test
  @DisplayName(
      "Case 2: Should receive the publisher notification when an recipe is updated, and call the notificator successfully.")
  void aoAtualizarReceitaSuccess() {
    ...
    // Verifica se o método 'enviar' foi acionado com os dados esperados
    verify(notificadorMock, times(1))
        .enviar(eq(usuarioMock.getLogin()), eq("Receita Atualizada"), any(String.class));
  }
